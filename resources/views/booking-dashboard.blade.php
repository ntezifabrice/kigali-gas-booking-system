                            ...
                            <div class="col-sm-4"><!-- list start -->
                                <div class="list-box-listing">
                                    <div class="list-box-listing-img">
                                        <a href="{{url('/dashboard/booking/'.$transaction->id)}}"><img src="{{$transaction->cylinder->image_url()}}" alt=""></a>
                                    </div>
                                    <div class="list-box-listing-content">
                                        <div class="inner">
                                            <a href="{{url('/dashboard/booking/'.$transaction->id)}}"><h3>{{$transaction->cylinder->name}}</h3></a>
                                            <span>{{$transaction->cylinder->price}}</span>
                                            <p><span class="font600">Agent: </span> {{$transaction->cylinder->user->name}}<br/>
                                                <span class="font600">Cylinder: </span> {{$transaction->cylinder->size}}<br/>
                                                <span class="font600">Status: </span> {{$transaction->status}}<br/>
                                            </p>
                                        </div>
                                        <a href="{{url('/dashboard/booking/'.$transaction->id)}}" class="button gray"><i class="sl sl-icon-eye"></i> View</a>
                                    </div>
                                </div>
                            </div><!-- list end -->
                        @empty
